...
test('a `soon` item renders as a dead span with a Soon pill, not a link', () => {
    ...
    // Purchase Order is still `soon: true` (Sales Order went live in task 3)
    // — and it must NOT be clickable.
    assert.doesNotMatch(html, /<a class="sidebar-link[^"]*"[^>]*>\s*<i[^>]*><\/i>\s*<span class="sidebar-link-text">Purchase Order</);
});
