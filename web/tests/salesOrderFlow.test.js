...
// sales-order.js को बिना DOM के चलाओ — सिर्फ़ गणित वाले हिस्से की जाँच।
function loadCalc() {
    const src = fs.readFileSync(path.join(__dirname, '..', 'public', 'js', 'sales-order.js'), 'utf8');
    const sandbox = { window: {}, document: { addEventListener() {}, querySelector: () => null, querySelectorAll: () => [] } };
    sandbox.globalThis = sandbox;
    vm.createContext(sandbox);
    vm.runInContext(src, sandbox);
    return sandbox.window.SalesOrderCalc;
}
...
test('buildVoucherNo trims the parts and skips the empty ones', () => {
    const { buildVoucherNo } = loadCalc();
    assert.strictEqual(buildVoucherNo({ prefix: ' SO/ ', number: '007', suffix: '' }), 'SO/007');
