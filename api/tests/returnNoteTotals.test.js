...
const { computeTotals } = require('../Controllers/Tenant/ReturnNoteController');
...
test('the voucher type strings are exactly what Tally uses', () => {
    const { VOUCHER_TYPE } = require('../Controllers/Tenant/ReturnNoteController');
    assert.strictEqual(VOUCHER_TYPE.credit, 'Credit Note');
    assert.strictEqual(VOUCHER_TYPE.debit, 'Debit Note');
});

test('each kind maps to the right invoice type', () => {
    const { INVOICE_TYPE } = require('../Controllers/Tenant/ReturnNoteController');
    assert.strictEqual(INVOICE_TYPE.credit, 'sales');
    assert.strictEqual(INVOICE_TYPE.debit, 'purchase');
});
