...
/**
 * api/Validators/purchaseOrder.js
 *
 * Joi schemas for the purchase-orders resource. Mirrors salesOrder.js — see
 ...
 * totals are computed by PurchaseOrderController.computeTotals inside the
 ...
 * `sales_person_id`; purchasing is not tied to a salesman.
 *
 * Schemas:
 *   createPurchaseOrderSchema — POST/PUT /purchase-orders (supplier_id required)
 *   listPurchaseOrderSchema   — GET  /purchase-orders       (pagination + filters)
 */
...
/**
 * A single purchase-order line. The money columns (taxable / gst_amount /
...
/**
 * POST /api/v1/purchase-orders (also used for PUT — the shape is the same).
 * `supplier_id` and at least one item are required; `order_no` is optional —
...
/**
 * GET /api/v1/purchase-orders (query string)
 ...
 * purchase-order-only delivery-lifecycle filter (pending/partially_delivered/
