...
/**
 * api/Validators/supplier.js
 *
 * Joi schemas for the suppliers resource. Mirrors the `suppliers` table
 * (migration 20260101000012) — the fields a client may set. Server-managed
 ...
 *
 * Schemas:
 *   createSupplierSchema — POST  /suppliers   (name required; rest optional)
 *   updateSupplierSchema — PUT   /suppliers/:id (all optional; ≥1 enforced)
 *   listSupplierSchema   — GET   /suppliers   (query: pagination + filters)
 *
 * Conventions:
 ...
 *   • the money field (opening_balance) is a non-negative number.
 *   • `status` is constrained to the Active | Inactive | Blocked set the table
 *     defaults around.
 *   • location_id is a positive integer; existence is enforced by the DB FK,
 *     not here.
 */

const Joi = require('joi');

// Allowed lifecycle states — matches the suppliers.status default ('Active').
...
const createSupplierSchema = Joi.object({
    name: Joi.string().trim().min(1).max(191).required().messages({
        ...
        'string.max':   'Supplier name is too long.',
    }),
...
/**
 * GET /api/v1/suppliers (query string)
